<section id="sec_segguros">
    <div class="col-md-12">
        <center>
            <label class="title-secction" for="">Ponemos a tú disposición nuestros servicios</label>
        </center>
        <div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner" id="seguros">
                <div class="carousel-item active">
                    <div class="row">
                        <div class="target-seguro seguro_1">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE GASTOS MÉDICOS</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_2">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE VIDA</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_3">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE AUTO</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_4">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE HOGAR</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_5">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE AHORRO</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="carousel-item">
                    <div class="row">
                        <div class="target-seguro seguro_5">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO 6</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_4">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE VIDA</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_3">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE AUTO</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_2">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE HOGAR</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                        <div class="target-seguro seguro_1">
                            <div class="title-target">
                                <label for="title-seguro">SEGURO DE AHORRO</label>
                            </div>
                            <div class="image-seguro">
                                <center>
                                    <img src="assets/img/chico.svg" alt="" class="img-target">
                                </center>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <a class="carousel-control-prev prev-seguros" href="#carouselExampleControls" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="carousel-control-next next-seguros" href="#carouselExampleControls" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Next</span>
        </a>
    </div>
</section>